...
func _test_requires_four_active_intents_initially() -> void:
	...
	_assert_eq(
		result.get("reason"), "incomplete_exchange", "missing fourth intent must be explicit"
	)
...
func _test_partial_ko_continues_with_three_survivors() -> void:
	...
	_assert_eq(
		invalid.get("reason"), "inactive_actor_intent", "KO actor rejection must be explicit"
	)
...
func _test_team_elimination_finishes_combat() -> void:
	...
	_assert_eq(
		result.get("status"), "combat_finished", "eliminating both beta fighters must finish"
	)
...
func _test_full_simultaneous_elimination_is_double_ko() -> void:
	...
	_assert_eq(
		(result.get("ko_fighter_ids", []) as Array).size(), 4, "all four fighters must be KO"
	)
