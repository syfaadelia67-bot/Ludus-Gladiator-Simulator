...
func _initialize() -> void:
    ...
    assert(arena_scene_text.contains("custom_minimum_size = Vector2(250, 0)"))
    assert(arena_scene_text.contains("custom_minimum_size = Vector2(285, 0)"))
    ...
    print("Arena balanced layout contract: OK")
    quit()
