...
func _initialize() -> void:
	...
	var shell_controller_text := FileAccess.get_file_as_string(
		"res://scripts/ui/unified_hud_shell.gd"
	)
	...
	assert(
		(
			instance.get_node_or_null(
				"ContentShell/PersonalView/ListPanel/Margin/Content/PersonalList"
			)
			is ItemList
		)
	)
	assert(
		(
			instance.get_node_or_null("ContentShell/FightersView/ModeButtons/GladiatorsButton")
			is Button
		)
	)
	assert(
		(
			instance.get_node_or_null(
				"ContentShell/FightersView/GladiatorsView/ListPanel/Margin/Content/GladiatorList"
			)
			is ItemList
		)
	)
	assert(
		(
			instance.get_node_or_null("ContentShell/FightersView/BeastsView/Content/Icon")
			is TextureRect
		)
	)
	instance.free()
