...
func run() -> void:
	var controller_source := FileAccess.get_file_as_string(
		"res://scripts/systems/gladiator_career_state_controller.gd"
	)
	var training_source := FileAccess.get_file_as_string(
		"res://scripts/systems/gladiator_training_controller.gd"
	)
	var combat_source := FileAccess.get_file_as_string(
		"res://scripts/systems/combat_manager_weekly.gd"
	)
	var presenter_source := FileAccess.get_file_as_string(
		"res://scripts/ui/gladiator_career_state_presenter.gd"
	)
	var project_source := FileAccess.get_file_as_string("res://project.godot")

	for state in ["activo", "veterano", "declive", "retirado"]:
		_assert(
			controller_source.contains('"%s"' % state),
			"Falta el estado de carrera %s." % state,
		)
	_assert(
	...
	_assert(
		controller_source.contains("retired_gladiators.push_front"),
		"El retiro debe conservar un resumen histórico.",
	)
	_assert(
		controller_source.contains("GladiatorCareerJournalController.add_event"),
		"El cambio de etapa y retiro deben registrarse en la historia.",
	)

	_assert(
		not training_source.contains("GladiatorCareerStateController.get_trainer_multiplier"),
		"El bonus de entrenador no puede aplicar mientras el balance mensual esté pendiente.",
	)
	_assert(
		not training_source.contains("GladiatorCareerStateController.get_mentor_morale_bonus"),
		"El bonus de mentor no puede aplicar mientras el balance mensual esté pendiente.",
	)
	_assert(
		training_source.contains("TRAINING_PROGRESS_ENABLED"),
		"Entrenamiento debe respetar la política mensual fail-closed.",
	)
	_assert(
		combat_source.contains("GladiatorCareerStateController.get_combat_modifiers"),
	...
	_assert(
		presenter_source.contains("Retirar como mentor"),
		"La ficha debe ofrecer retiro como mentor.",
	)
	_assert(
		presenter_source.contains("El retiro es permanente"),
		"La interfaz debe advertir la irreversibilidad del retiro.",
	)
	_assert(
		not presenter_source.contains('box.name = "Carrera"'),
		"No debe agregarse otra pestaña a la ficha.",
	)
	...
	_assert(
		project_source.contains(
			'GladiatorCareerStatePresenter="*res://scripts/ui/gladiator_career_state_presenter.gd"'
		),
