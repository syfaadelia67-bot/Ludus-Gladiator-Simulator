...
func _initialize() -> void:
	var manager_text := FileAccess.get_file_as_string(
		"res://scripts/systems/relationship_manager.gd"
	)
	...
	assert(screen_text.contains('"INTERVENCIONES DEL MES %d/%d"'))
	...
	assert(
		(
			instance.get_node_or_null("Margin/Main/Body/PeoplePanel/Margin/Content/PeopleList")
			is ItemList
		)
	)
	assert(
		(
			instance.get_node_or_null("Margin/Main/Body/BondsPanel/Margin/Content/Scroll/BondCards")
			is VBoxContainer
		)
	)
	assert(
		(
			instance.get_node_or_null("Margin/Main/IncidentPanel/Margin/Content/Choices")
			is HBoxContainer
		)
	)
	assert(
		(
			instance.get_node_or_null("Margin/Main/Body/DetailPanel/Margin/Scroll/Content/Actions")
			is VBoxContainer
		)
	)
	assert(
		(
			instance.get_node_or_null("Margin/Main/Body/DetailPanel/Margin/Scroll/Content/Status")
			is Label
		)
	)
	instance.free()
...
func _test_intervention_changes_gameplay_state() -> void:
	...
	var result_value = relationship_manager.call(
		"register_interaction", a_id, b_id, "train_together"
	)
