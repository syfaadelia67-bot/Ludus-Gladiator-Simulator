...
func run() -> void:
	...
	assert(economy_script.contains('contract["unfrozen_actions_fail_closed"] = true'))
	...
	assert(panel_script.contains("fuera de demo"))
	assert(panel_script.contains("sponsor_selector.disabled = true"))
	assert(panel_script.contains("loan_selector.disabled = true"))
