...
func run() -> void:
	...
	assert(not hub.contains("tabs.visible"))
	assert(not hub.contains("TabContainer"))
	assert(
		finca_scene.contains('name="FincaScreen"') or finca_scene.contains('name = "FincaScreen"')
	)
