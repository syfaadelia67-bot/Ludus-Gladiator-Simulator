...
func _assert_active_monthly_sources() -> void:
	...
	assert(screen.contains("MERCADO DE LUCHADORES · ACTUALIZACIÓN MENSUAL"))
	assert(screen.contains("RENOVACIÓN MANUAL NO DISPONIBLE"))
	assert(screen.contains("En la demo, el equipamiento se obtiene mediante la Forja."))
	assert(not screen.contains("PENDIENTE DE BALANCE"))
	assert(not screen.contains("hasta congelar"))
