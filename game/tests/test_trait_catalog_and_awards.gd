...
func _ready() -> void:
	...
	assert(
		not TraitManager.award_trait(gladiator.id, "disciplined"),
		"Duplicate traits must be rejected"
	)
	assert(
	...
	assert(
		not TraitManager.award_trait(gladiator.id, "dreamer"), "Removed legacy traits must be inert"
	)
