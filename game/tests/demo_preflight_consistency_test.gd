...
func _assert_ability_catalog() -> void:
	assert(
		...
		"The demo must expose exactly eight canonical abilities"
	)
	...
	for raw_entry in DataRepository.abilities:
		...
		assert(
			int(entry.get("demo_max_level", 0)) == 2,
			"%s must stop at rank II in the demo" % ability_id
		)
		assert(
			int(entry.get("full_max_level", 0)) == 3,
			"%s must reserve rank III for the full game" % ability_id
		)
		var levels: Dictionary = entry.get("levels", {})
		assert(
			levels.has("1") and levels.has("2") and levels.has("3"),
			"%s must define ranks I, II and III" % ability_id
		)
		var third_rank: Dictionary = levels.get("3", {})
		assert(bool(third_rank.get("locked", false)), "%s rank III must remain locked" % ability_id)
		assert(
			str(third_rank.get("status", "")) == "coming_soon",
			"%s rank III must be marked coming soon" % ability_id
		)
	found.sort()
...
func _assert_specialization_catalog() -> void:
	assert(
		...
		"The canonical catalog must contain the generic class plus four specializations"
	)
	var found: Array[String] = []
	for raw_entry in DataRepository.specializations:
		...
		if EXPECTED_CLASS_ABILITIES.has(specialization_id):
			assert(
				str(entry.get("class_ability", "")) == EXPECTED_CLASS_ABILITIES[specialization_id],
				"%s must reference its canonical class ability" % specialization_id
			)
	found.sort()
...
func _assert_equipment_contracts() -> void:
	...
	assert(
		...
		"Embate del escudo must require a shield"
	)
	assert(
		cast_net.get("required_equipment_tags", []).has("net"), "Red de captura must require a net"
	)
	assert(
