...
func _initialize() -> void:
	...
	assert(
		(
			arena_scene.contains('name="PlayerFighter"')
			or arena_scene.contains('name = "PlayerFighter"')
		)
	)
	assert(
		arena_scene.contains('name="EnemyFighter"') or arena_scene.contains('name = "EnemyFighter"')
	)
	assert(arena_scene.contains('name="EffectIcon"') or arena_scene.contains('name = "EffectIcon"'))
	assert(
		arena_scene.contains('name="StatusIcons"') or arena_scene.contains('name = "StatusIcons"')
	)
	assert(
		(
			arena_scene.contains('name="EquipmentIcons"')
			or arena_scene.contains('name = "EquipmentIcons"')
		)
	)
	assert(
		(
			arena_scene.contains('name="ResultEffects"')
			or arena_scene.contains('name = "ResultEffects"')
		)
	)
