...
func _ready() -> void:
	...
	var bridge := FileAccess.get_file_as_string("res://scripts/combat/gt1_combat_intent_bridge.gd")
	var closure := FileAccess.get_file_as_string("res://scripts/ui/weekly_closure_presenter.gd")
	var panel := FileAccess.get_file_as_string("res://scripts/ui/combat_history_panel.gd")
	...
	assert(bridge.contains("CombatHistoryManager"))
	assert(bridge.contains("register_gt1_bout("))
	...
	assert(panel.contains("func _entry_month"))
	assert(panel.contains("Mes %d"))
	assert(not panel.contains("Semana %d"))
	...
	assert(
		not closure.contains("La recuperación automática permanece pendiente de balance mensual.")
	)
