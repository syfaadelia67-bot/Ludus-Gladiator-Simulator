...
func run() -> void:
	...
	record["active_injury"] = {
		"name": "Fractura grave",
		"severity": 3,
		# Save-v14 compatibility keys remain unchanged until a schema migration exists.
		"started_week": 2,
		"recovery_weeks": 3,
	...
	_assert(
		fighter.get_injury_summary().contains("mes"),
		"La recuperación activa debe presentarse en meses."
	)
	_assert(
		int(GladiatorInjuryController.get_active_injury(fighter.id).get("severity", 0)) == 3,
		"La gravedad activa debe conservarse."
	)
	...
	_assert(
		str(scars[0].get("name", "")).contains("fractura"),
		"La fractura grave debe producir una secuela identificable."
	)
	_assert(
		fighter.agility == agility_before - 1,
		"La secuela de fractura debe reducir Agilidad en un punto."
	)
	_assert(
		GladiatorInjuryController.get_active_injury(fighter.id).is_empty(),
		"La lesión activa debe limpiarse al completar la recuperación."
	)
	...
	_assert(
		GladiatorInjuryController.get_scars(fighter.id).size() == 1,
		"Las cicatrices deben persistir en el guardado de progresión."
	)

	var legacy_combat_source := FileAccess.get_file_as_string(
		"res://scripts/systems/combat_manager_weekly.gd"
	)
	_assert(
		legacy_combat_source.contains("WEEKLY_INJURIES"),
		"El CombatManager legacy debe conservar su catálogo hasta ser puesto en cuarentena."
	)
	_assert(
		legacy_combat_source.contains("severity = 3"),
		"El catálogo legacy debe conservar lesiones graves durante la compatibilidad."
	)

	var project_source := FileAccess.get_file_as_string("res://project.godot")
	_assert(
		project_source.contains(
			'GladiatorInjuryController="*res://scripts/systems/gladiator_injury_controller.gd"'
		),
		"El controlador de lesiones debe estar registrado como autoload."
	)

	var presenter_source := FileAccess.get_file_as_string(
		"res://scripts/ui/gladiator_career_journal_presenter.gd"
	)
	_assert(presenter_source.contains("ESTADO MÉDICO"), "La ficha debe mostrar el estado médico.")
	_assert(
		presenter_source.contains("Cicatrices permanentes"),
		"La ficha debe mostrar cicatrices permanentes."
	)
