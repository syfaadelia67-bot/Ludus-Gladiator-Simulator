...
func run() -> void:
	...
	assert(manager_source.contains("return []"))
	assert(manager_source.contains("return [_build_gt1_event(month)]"))
	assert(manager_source.contains("_quarantine_legacy_non_gt_contracts"))
	assert(not manager_source.contains("_build_underworld_event(month)"))
	assert(not manager_source.contains("_build_minor_event(month"))
