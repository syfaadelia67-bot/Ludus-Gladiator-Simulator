...
func _initialize() -> void:
	...
	for required_node in [
		"TopHUD", "MainNavigation", "WorldArea", "BuildingDetailsPanel", "Scroll", "BottomStatusBar"
	]:
		assert(
			(
	...
	for required_shell_node in [
		...
		assert(
			(
				shell_scene_text.contains('name="%s"' % required_shell_node)
				or shell_scene_text.contains('name = "%s"' % required_shell_node)
			)
		)
	...
	assert(
		(
			finca_instance.get_node_or_null("Center/BuildingDetailsPanel/Margin/Scroll")
			is ScrollContainer
		)
	)
	finca_instance.free()
