...
func _initialize() -> void:
	...
	var equipment_catalog_text := FileAccess.get_file_as_string("res://data/weapons.json")
	...
	var market_manager_text := FileAccess.get_file_as_string(
	...
	for tab_node in [
		"Information", "Statistics", "Equipment", "Skills", "Specialization", "Traits", "Bonds"
	]:
		assert(
			(
				scene_text.contains('name="%s"' % tab_node)
				or scene_text.contains('name = "%s"' % tab_node)
			)
		)
	...
	assert(equipment_text.contains("DataRepository.weapons"))
	assert(equipment_catalog_text.contains('"id": "antidote_kit"'))
	assert(equipment_catalog_text.contains('"id": "hidden_hook"'))
	assert(equipment_catalog_text.contains('"id": "linen_wraps"'))
	...
	assert(
		market_screen_text.contains(
			"Los objetos se equipan desde la ficha individual del gladiador"
		)
	)
	...
	assert(
		save_text.contains('"equipment":{"inventory":EquipmentManager.inventory.duplicate(true)')
	)
	...
	assert(
		(
			instance.get_node_or_null("Margin/Main/Body/ContentPanel/Margin/Layout/Scroll/Content")
			is VBoxContainer
		)
	)
	instance.free()
