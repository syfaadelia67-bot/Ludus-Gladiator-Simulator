...
func run() -> void:
	...
	assert(modal.contains("MonthlyEventModal"))
	assert(not modal.contains("WeeklyEventModal"))
	...
	assert(modal.contains("EVENTO MENSUAL · MES"))
	...
	# Autoload name remains stable for scene/save compatibility.
	assert(project.contains("WeeklyEventModalPresenter="))
	print("Monthly event modal and hosted Arena exit contract: OK")
