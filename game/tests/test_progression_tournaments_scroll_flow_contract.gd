...
func _initialize() -> void:
	var progression_scene_text := FileAccess.get_file_as_string(
		"res://scenes/ProgressionPanel.tscn"
	)
	var tournaments_scene_text := FileAccess.get_file_as_string(
		"res://scenes/TournamentsPanel.tscn"
	)
	var monthly_tournaments_scene_text := FileAccess.get_file_as_string(
		"res://scenes/TournamentsPanelMonthly.tscn"
	)
	var progression_script_text := FileAccess.get_file_as_string(
		"res://scripts/ui/progression_panel.gd"
	)
	var tournaments_script_text := FileAccess.get_file_as_string(
		"res://scripts/ui/tournaments_panel.gd"
	)
	...
	assert(
		progression_scene_text.contains(
			'[node name="BackToFinca" type="Button" parent="Navigation"]'
		)
	)
	...
	assert(
		tournaments_scene_text.contains(
			'[node name="BackToFinca" type="Button" parent="Navigation"]'
		)
	)
	...
	assert(monthly_tournaments_scene_text.contains("tournaments_panel_monthly.gd"))

	assert(hub_text.contains('"torneos": "res://scenes/TournamentsPanelMonthly.tscn"'))
	...
	var tournaments_scene := load("res://scenes/TournamentsPanelMonthly.tscn") as PackedScene
	...
	assert(
		(
			tournaments_instance.get_node_or_null(
				"Scroll/Content/TournamentContent/Right/Contracts"
			)
			!= null
		)
	)
	...
	print("Progression and monthly tournaments scroll flow contract: OK")
	quit()
