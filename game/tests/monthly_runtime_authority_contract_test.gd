...
func _assert_non_gt_tournament_authority() -> void:
	...
	assert(tournaments.contains("_build_underworld_event(month)"))
	assert(tournaments.contains("_build_minor_event(month, 1)"))
	assert(tournaments.contains("_build_gt1_event(month)"))
	assert(tournaments.contains("CANONICAL_COMPETITIONS"))
	assert(tournaments.contains("get_non_gt_activity_contract"))
	assert(not tournaments.contains("_quarantine_legacy_non_gt_contracts"))
...
func _assert_roster_training_recovery_authority() -> void:
	...
	assert(policy.contains('MIGRATION_MODE := "one_legacy_turn_equals_one_monthly_turn"'))
	assert(policy.contains("WORK_OUTPUTS_ENABLED := true"))
	assert(policy.contains("TRAINING_PROGRESS_ENABLED := true"))
	assert(policy.contains("FATIGUE_MUTATION_ENABLED := true"))
	assert(policy.contains("INJURY_AUTO_RECOVERY_ENABLED := true"))
	assert(policy.contains("FATIGUE_COMBAT_LIMIT := 90"))
	assert(policy.contains("SLAVE_PROMOTION_TRAINING_THRESHOLD := 100"))
	assert(person.contains("func process_month()"))
	assert(person.contains("MONTHLY_ROSTER_WORK_POLICY.MINING_FATIGUE"))
	assert(person.contains("MONTHLY_ROSTER_WORK_POLICY.TRAINING_FATIGUE"))
	assert(person.contains("training += gained"))
	assert(person.contains("injury_days = maxi(0, injury_days - 1 - recovery_bonus)"))
	assert(person.contains("func process_day()"))
	assert(person.contains("return process_month()"))
