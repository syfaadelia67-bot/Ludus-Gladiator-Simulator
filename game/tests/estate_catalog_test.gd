...
func _ready() -> void:
    ...
    assert(estate.BUILDINGS.size() == 7, "The frozen estate catalog must define exactly seven facilities")
    assert(estate.get_building_ids() == CANONICAL_BUILDINGS, "The estate catalog must expose only the seven frozen facilities")
    assert(estate.canonicalize_building_id("quarters") == "barracks", "Legacy quarters id must migrate to barracks")
    assert(estate.canonicalize_building_id("guard_post").is_empty(), "Removed guard post ids must not remain canonical")
    assert(estate.canonicalize_building_id("security").is_empty(), "Removed security building ids must not remain canonical")
    assert(estate.canonicalize_building_id("kitchen").is_empty(), "Removed kitchen ids must not remain canonical")
    ...
    assert(not migrated.has("wall_and_gate"), "Removed security buildings must not enter canonical saves")
    ...
    assert(migrated.size() == 7, "Migrated estate state must contain every frozen facility and nothing else")
    ...
    assert(estate.get_effective_max_level("forge") == 3, "Demo buildings must allow progression through level III")
    assert(estate.get_effective_max_level("mine") == 3, "Mine must use the same demo level cap")
    assert(not estate.can_upgrade("mine"), "Mine upgrades must remain blocked until its frozen monthly cost is recovered")
    assert(estate.get_security_bonus() == 0, "Security must no longer come from a building")

    estate.demo_mode = false
    assert(estate.get_effective_max_level("forge") == 10, "Full game facilities must preserve level X progression")
    assert(estate.get_effective_max_level("beast_area") == 10, "Beast area must preserve level X progression")
