...
func _ready() -> void:
	var owner_source := FileAccess.get_file_as_string(
		"res://scripts/systems/ludus_owner_manager.gd"
	)
	...
	assert(owner_source.contains("DataRepository.get_economy_rule"))
	assert(owner_source.contains('starting_resources.get("denarii", 0)'))
	...
	assert(
		(
			owner_source.find("_reset_campaign_for_new_owner()")
			< owner_source.find("_apply_origin_bonuses_once()")
		)
	)
