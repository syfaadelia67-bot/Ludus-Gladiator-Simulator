...
func run() -> void:
	...
	_assert(
		...
		"Deben existir gladiador y cuatro especializaciones."
	)
	_assert(
		TraitManager.get_normal_trait_ids().size() == 16,
		"Deben existir dieciséis rasgos normales canónicos."
	)
	_assert(
		TraitManager.get_origin_trait_ids().is_empty(),
		"Los rasgos de origen legacy deben permanecer inactivos."
	)
	_assert(
		TraitManager.get_obtainable_trait_ids().is_empty(),
		"Los rasgos obtenibles legacy deben permanecer inactivos."
	)
	...
	_assert(
		int(fresh.get("skill_points", 0)) == 1,
		"Un gladiador nuevo debe comenzar con un punto de habilidad."
	)
	_assert(
		fresh_abilities.is_empty(),
		"Un gladiador nuevo no debe comenzar con habilidades aprendidas."
	)
	...
	_assert(
		str(migrated.get("specialization", "")) == "dimachaerus",
		"thraex debe migrar a dimachaerus."
	)
	...
	_assert(
		int(migrated_abilities.get("throw_sand", 0)) == 1,
		"Reservas profundas debe migrar a Arrojar arena."
	)
	...
	_assert(
		GladiatorProgressionManager.set_tactical_plan(fighter.id, valid_plan),
		"El plan táctico válido debe guardarse."
	)
	_assert(
		GladiatorProgressionManager.get_tactical_plan(fighter.id).size() == 3,
		"El plan táctico debe conservar sus órdenes."
	)
	...
	_assert(
		serialized.has("applied_trait_effects"),
		"La ficha guardada debe registrar efectos permanentes aplicados."
	)
	...
	for trait_id in offer_traits:
		_assert(
			canonical_traits.has(str(trait_id)),
			"Las ofertas nuevas solo pueden usar rasgos normales canónicos."
		)
