...
func _ready() -> void:
	...
	assert(contract.get("authority") == "monthly_market_policy")
	assert(contract.get("month_native") == true)
	assert(contract.get("authored_unique_sync_enabled") == true)
	assert(int(contract.get("authored_unique_sync_cadence_months", 0)) == 1)
	assert(contract.get("procedural_auto_rotation_enabled") == false)
	...
	assert(policy.should_sync_authored_unique_offers(2, 1))
	assert(not policy.should_sync_authored_unique_offers(2, 2))
	...
	print("Monthly market policy frozen contract: OK")
	get_tree().quit(0)
