...
func run() -> void:
	...
	assert(hud.contains("GameState.month_advanced.connect"))
	assert(not hud.contains("GameState.week_advanced.connect"))
	...
	print("Main controller removal and monthly hosted ownership contract: OK")
