...
func start(state: Dictionary) -> Dictionary:
	...
	if str(state.get("format", "")) != "1v2":
		return _rejected("unsupported_format", ["Combat1v2Loop requires format 1v2"], state, 0)
	return {
		"status": "running",
		"errors": [],
		"exchange_index": 0,
...
func advance(loop_state: Dictionary, intents: Array) -> Dictionary:
	...
	if session.get("status") != "resolved":
		return _rejected(
			"incomplete_exchange",
			["1v2 loop requires exactly one valid intent from each active fighter"],
			state,
			int(loop_state.get("exchange_index", 0)),
		)
...
func _validate_loop_state(loop_state: Dictionary) -> Array[String]:
	...
	if str(loop_state.get("status", "")) != "running":
		errors.append("1v2 loop must be running before advance")
	if not loop_state.get("state", {}) is Dictionary:
		errors.append("1v2 loop state must contain a CombatState Dictionary")
	if not errors.is_empty():
		return errors
	var state := loop_state.get("state", {}) as Dictionary
	errors.append_array(_combat_contract.validate_state(state))
	if str(state.get("format", "")) != "1v2":
		errors.append("1v2 loop state format must remain 1v2")
	return errors
