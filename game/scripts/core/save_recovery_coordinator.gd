...
var pending_runtime_recovery: bool = false
...
func _on_load_completed(loaded_path: String) -> void:
	...
	var runtime_report := CombatV1SessionStore.get_last_import_report()
	var runtime_recovered := bool(runtime_report.get("legacy_partial_gt1_recovered", false))
	if not recovered_from_backup and not requires_migration and not runtime_recovered:
		return
	...
	if recovered_from_backup and FileAccess.file_exists(PRIMARY_PATH):
		...
		if removal_error != OK:
			var removal_message := (
				"La campaña se cargó desde la copia de seguridad, pero no se pudo retirar "
				+ "el archivo principal dañado."
			)
	...
	pending_runtime_recovery = runtime_recovered
...
func _rewrite_loaded_campaign() -> void:
	...
	if not SaveManager.save_game():
		var failure_message := "La campaña cargada no pudo reescribirse con el formato actual."
...
func _completion_message() -> String:
	if pending_runtime_recovery and pending_recovered_from_backup:
		return (
			"Campaña recuperada desde la copia de seguridad; el encuentro GT I incompleto "
			+ "se reinició de forma segura y el guardado fue actualizado."
		)
	if pending_runtime_recovery:
		return (
			"El encuentro GT I incompleto de una partida anterior se reinició de forma "
			+ "segura y el guardado fue actualizado."
		)
	if pending_recovered_from_backup and pending_requires_migration:
		return (
			"Campaña recuperada desde la copia de seguridad y actualizada al formato de "
			+ "guardado actual."
		)
