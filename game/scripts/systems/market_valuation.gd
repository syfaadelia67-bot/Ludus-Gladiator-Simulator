...
static func offer_value(offer: Dictionary) -> int:
    var attributes := int(offer.get("strength", 5)) + int(offer.get("agility", 5)) + int(offer.get("endurance", 5)) + int(offer.get("intelligence", 5)) + int(offer.get("technique", 5))
    var value := 90 + attributes * ATTRIBUTE_VALUE
...
static func person_value(person, progression: Dictionary = {}) -> int:
    if person == null:
        return 0
    var attributes := int(person.strength) + int(person.agility) + int(person.endurance) + int(person.intelligence) + int(person.technique)
    var value := attributes * ATTRIBUTE_VALUE + maxi(1, int(person.health)) * HEALTH_POINT_VALUE
