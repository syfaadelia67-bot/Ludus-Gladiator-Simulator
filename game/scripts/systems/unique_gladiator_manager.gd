...
func _connect_runtime_signals() -> void:
	...
	if not NewCampaignCoordinator.campaign_reset_completed.is_connected(
		_on_campaign_reset_completed
	):
...
func reconcile_from_world() -> void:
	...
	for raw_offer in MarketManager.offers:
		...
		if gladiator_id.is_empty() or not states.has(gladiator_id):
			continue
		var entry := DataRepository.get_unique_gladiator(gladiator_id)
		if bool(entry.get("initial_candidate", false)):
			continue
		var state: Dictionary = states[gladiator_id]
		state["status"] = "market"
...
func get_available_market_offers() -> Array[Dictionary]:
	...
	for entry in DataRepository.unique_gladiators:
		...
		offer["weeks_remaining"] = maxi(
			0, int(state.get("expires_week", 0)) - GameState.get_week() + 1
		)
		result.append(offer)
	return result
...
func acquire_initial_gladiator(gladiator_id: String) -> bool:
	...
	if str(state.get("status", "")) != "initial_market":
		return false
	var rival_ids := _canonical_rival_ids()
	...
	for other_id in states.keys():
		if other_id == gladiator_id:
			continue
		if str(states[other_id].get("status", "")) != "initial_market":
			continue
		_move_to_rival(
			str(other_id), rival_ids[rival_index % rival_ids.size()], GameState.get_week()
		)
		rival_index += 1
...
func _sanitize_states() -> void:
	...
	for gladiator_id in states.keys():
		...
		if status not in ["initial_market", "market", "player", "rival", "locked"]:
			status = "locked"
		var rival_id := str(state.get("rival_id", ""))
		if status == "rival":
			rival_id = _canonicalize_rival_id(rival_id, str(gladiator_id))
...
func _canonical_rival_ids() -> Array[String]:
	var result: Array[String] = []
	for raw_rival in DataRepository.get_rival_ludi():
		if raw_rival is Dictionary:
			var rival_id := str((raw_rival as Dictionary).get("id", ""))
			if not rival_id.is_empty():
				result.append(rival_id)
	result.sort()
	return result


func _canonicalize_rival_id(rival_id: String, gladiator_id: String) -> String:
	if not DataRepository.get_rival_ludus(rival_id).is_empty():
		return rival_id
	var aliased := str(LEGACY_RIVAL_ID_ALIASES.get(rival_id, ""))
	if not aliased.is_empty() and not DataRepository.get_rival_ludus(aliased).is_empty():
		return aliased
	return _select_rival_for(gladiator_id)
...
func _select_rival_for(gladiator_id: String) -> String:
	...
	if rival_ids.is_empty():
		return ""
	return rival_ids[absi(hash("%s|%d" % [gladiator_id, GameState.get_week()])) % rival_ids.size()]


func _move_to_rival(gladiator_id: String, rival_id: String, week: int) -> void:
	var canonical_rival_id := _canonicalize_rival_id(rival_id, gladiator_id)
	if canonical_rival_id.is_empty():
		return
	var state: Dictionary = states.get(gladiator_id, {})
	state["status"] = "rival"
	state["rival_id"] = canonical_rival_id
	state["acquired_week"] = week
	states[gladiator_id] = state
	_assign_to_rival(canonical_rival_id, gladiator_id)
	unique_gladiator_claimed_by_rival.emit(gladiator_id, canonical_rival_id)
