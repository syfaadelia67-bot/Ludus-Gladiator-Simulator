...
func _evaluate_objectives() -> void:
    ...
    for objective in OBJECTIVES:
        ...
        if completed_objectives.has(objective_id) or failed_objectives.has(objective_id):
            continue
        if GameState.get_week() > _deadline_for_objective(objective):
...
func get_objectives(chapter_id: String = "") -> Array:
    ...
    for objective in OBJECTIVES:
        ...
        data["failed"] = failed_objectives.has(objective_id) or (GameState.get_week() > deadline and not bool(data["completed"]))
        data["deadline_week"] = deadline
        data["weeks_remaining"] = maxi(0, deadline - GameState.get_week() + 1)
        result.append(data)
    return result
...
func import_state(data: Dictionary) -> void:
    ...
    for raw_id in data.get("failed_objectives", []):
        ...
        if _objective_exists(objective_id) and not completed_objectives.has(objective_id) and not failed_objectives.has(objective_id):
...
func _fail_objective(objective: Dictionary) -> void:
    var objective_id := str(objective.get("id", ""))
    if objective_id.is_empty() or completed_objectives.has(objective_id) or failed_objectives.has(objective_id):
        return
    failed_objectives.append(objective_id)
    var failed := objective.duplicate(true)
    failed["deadline_week"] = _deadline_for_objective(objective)
...
func _deadline_for_objective(objective: Dictionary) -> int:
    ...
    for chapter in CHAPTERS:
        if str(chapter.get("id", "")) == chapter_id:
            return int(chapter.get("week_end", DEMO_FINAL_WEEK))
    return DEMO_FINAL_WEEK
