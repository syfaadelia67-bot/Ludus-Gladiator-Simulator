...
func simulate_duel(gladiator_id: String, tactic: String = "balanced") -> Dictionary:
    ...
    while player.health > 0 and enemy.health > 0 and round < 30:
        round += 1
        _perform_attack(player, enemy, rng, log)
        if enemy.health <= 0:
            break
        _perform_attack(enemy, player, rng, log)
