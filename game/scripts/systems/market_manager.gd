...
func refresh_market(charge: bool = true) -> bool:
    if CampaignManager.campaign_over:
        ...
        return false
    if charge:
        purchase_failed.emit("Las ofertas de luchadores se renuevan automáticamente cada 3 semanas.")
        return false
...
func refresh_equipment_market(charge: bool = true) -> bool:
    if CampaignManager.campaign_over:
        equipment_purchase_failed.emit("La campaña terminó. El mercado está disponible solo para consulta.")
        return false
    if charge and not GameState.spend_denarii(EQUIPMENT_REFRESH_COST):
        equipment_purchase_failed.emit("Se necesitan %d denarios para renovar el equipamiento." % EQUIPMENT_REFRESH_COST)
        return false
...
func buy_offer(offer_id: String) -> bool:
    ...
    var confirmed := true
    if not unique_id.is_empty():
        confirmed = UniqueGladiatorManager.acquire_initial_gladiator(unique_id) if unique_status == "initial_market" else UniqueGladiatorManager.acquire_market_gladiator(unique_id)
    if not confirmed:
