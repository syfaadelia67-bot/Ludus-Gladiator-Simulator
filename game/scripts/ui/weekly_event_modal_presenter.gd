...
func _ready() -> void:
	...
	SaveManager.load_completed.connect(
		func(_path: String): call_deferred("_show_pending_event_if_needed")
	)
	NewCampaignCoordinator.campaign_reset_completed.connect(
		func(): call_deferred("_show_pending_event_if_needed")
	)
...
func _attach_when_ready() -> void:
	for _attempt in range(60):
		...
		return
	push_error("No se pudo montar la ventana de evento mensual.")
...
func _show_event(event: Dictionary) -> void:
	...
	var month := int(event.get("month", event.get("week", GameState.get_month())))
	var chapter := CampaignManager.get_chapter_for_month(month)

	var kicker := Label.new()
	kicker.text = "EVENTO MENSUAL · MES %d" % month
	...
	for choice_variant in event.get("choices", []):
		...
		consequence.text = (
			...
			else _describe_effects(choice.get("effects", {}))
		)
		consequence.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		card.add_child(consequence)

	feedback = Label.new()
	feedback.text = "El mes no puede continuar sin una decisión."
...
func _show_result(result: Dictionary) -> void:
	...
	var kicker := Label.new()
	var month := int(result.get("month", result.get("week", GameState.get_month())))
	kicker.text = "DECISIÓN REGISTRADA · MES %d" % month
	kicker.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	card.add_child(kicker)
...
func _ensure_modal(scene: Node) -> void:
	...
	overlay.name = "MonthlyEventModal"
...
func _describe_effects(effect_data: Dictionary) -> String:
	var labels := {
		...
		"security": "Seguridad",
		"morale_all": "Moral de todos",
		"loyalty_all": "Lealtad de todos",
		"training_all": "Entrenamiento de todos"
	}
	...
	if timed is Dictionary and not timed.is_empty():
		var duration := int(timed.get("months", timed.get("weeks", 1)))
		parts.append("%s durante %d mes(es)" % [timed.get("name", "Efecto temporal"), duration])
	return (
		"Consecuencias: %s" % ", ".join(parts)
		if not parts.is_empty()
		else "Sin costo ni efecto directo"
	)
