...
func _layout_hotspots() -> void:
	...
	for entry: Dictionary in BUILDING_LAYOUT:
		...
		var desired_position := Vector2(
			area_size.x * float(entry.get("x", 0.0)), area_size.y * float(entry.get("y", 0.0))
		)
		desired_position.x = clampf(
			desired_position.x, 4.0, maxf(4.0, area_size.x - button_size.x - 4.0)
		)
		desired_position.y = clampf(
			desired_position.y, 72.0, maxf(72.0, area_size.y - button_size.y - 4.0)
		)
...
func _refresh_selected_building() -> void:
	...
	building_status.text = (
		"BLOQUEADA · JUEGO COMPLETO" if locked else "OPERATIVA · NIVEL %d/%d" % [level, max_level]
	)
...
func _refresh_alerts() -> void:
	...
	food_alert.text = (
		"COMIDA\n%d disponibles · %s"
		% [GameState.food, "RIESGO" if GameState.food < weekly_need else "Suficiente"]
	)
	workers_alert.text = (
