...
func _ready() -> void:
	...
	GladiatorMedicalCareController.priority_changed.connect(
		func(_person_id: String): call_deferred("_refresh_if_open")
	)
	GladiatorInjuryController.injury_state_changed.connect(
		func(_person_id: String): call_deferred("_refresh_if_open")
	)
...
func _on_treatment_purchased(
	person_id: String, treatment_id: String, months_reduced: int, cost: int
) -> void:
	if person_id != _get_open_person_id():
		return
	var treatment := GladiatorMedicalCareController.get_treatment(treatment_id, person_id)
	feedback_text = (
		"%s aplicado: -%d mes(es), costo %d denarios."
		% [treatment.get("name", treatment_id), months_reduced, cost]
	)
...
func _render(person_id: String) -> void:
	...
	status.text = (
		"Enfermería nivel %d · %s · gravedad %d · %d mes(es) registrados."
		% [
			infirmary_level,
			person.injury_name,
			person.injury_severity,
			person.get_injury_recovery_months(),
		]
	)
	section.add_child(status)

	var policy_status := Label.new()
	policy_status.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	policy_status.text = (
		"Balance mensual pendiente: tratamientos, costos, prioridad y velocidad de recuperación "
		+ "no aplican cambios numéricos todavía."
	)
	section.add_child(policy_status)
	...
	priority_button.text = (
		"Prioridad registrada"
		if GladiatorMedicalCareController.is_priority(person_id)
		else "Registrar prioridad"
	)
	...
	treatment_header.text = "Tratamientos · balance mensual pendiente"
	...
	for treatment_id in GladiatorMedicalCareController.get_treatment_ids():
		...
		button.tooltip_text = _treatment_tooltip(treatment)
...
func _set_priority(person_id: String) -> void:
	feedback_text = (
		"Prioridad médica registrada sin efecto numérico todavía."
		if GladiatorMedicalCareController.set_priority(person_id)
		else feedback_text
	)
	_refresh_if_open()


func _treatment_tooltip(treatment: Dictionary) -> String:
	...
	return "Efecto, costo y cadencia pendientes de balance mensual canónico."
