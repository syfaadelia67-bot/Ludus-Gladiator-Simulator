...
func _refresh() -> void:
	...
	var owner_identity := (
		owner_title if owner_name.is_empty() else "%s %s" % [owner_title, owner_name]
	)
	...
	if next_rank.is_empty():
		var rank_max_title := "[b]Rango máximo alcanzado[/b]\n"
		var rank_max_body := "El ludus ya obtuvo el mayor rango disponible."
		next_rank_label.text = rank_max_title + rank_max_body
	else:
		var next_rank_format := (
			"[b]Próximo rango: %s[/b]\nRequiere %d reputación, "
			+ "%d victorias y %d denarios.\nDesbloqueo: %s"
		)
...
func _add_objective_card(objective: Dictionary) -> void:
	...
	progress.text = (
		"Progreso: %d/%d" % [int(objective.get("progress", 0)), int(objective.get("target", 1))]
	)
	...
	reward.text = (
		"Recompensa: %d denarios y %d reputación"
		% [int(objective.get("reward_denarii", 0)), int(objective.get("reward_reputation", 0))]
	)
