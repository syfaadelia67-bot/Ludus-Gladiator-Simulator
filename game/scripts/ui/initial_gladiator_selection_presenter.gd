...
func _bind() -> void:
	...
	market_list = (
		hosted_market.get_node_or_null("ContentShell/FightersView/OffersPanel/Margin/Content/List")
		as ItemList
	)
	market_details = (
		hosted_market.get_node_or_null(
			"ContentShell/FightersView/DetailsPanel/Margin/Scroll/Content/Details"
		)
		as RichTextLabel
	)
	buy_button = (
		hosted_market.get_node_or_null(
			"ContentShell/FightersView/DetailsPanel/Margin/Scroll/Content/Buy"
		)
		as Button
	)
...
func _refresh() -> void:
	if market_panel == null or not is_instance_valid(market_panel):
		_bind()
		return
	var choosing_first: bool = (
		not UniqueGladiatorManager.first_purchase_completed and not RosterManager.has_gladiator()
	)
	...
	if market_landing != null:
		...
		market_landing.mouse_filter = (
			Control.MOUSE_FILTER_PASS if not choosing_first else Control.MOUSE_FILTER_IGNORE
		)
	if market_content_shell != null:
		market_content_shell.visible = not choosing_first
		market_content_shell.mouse_filter = (
			Control.MOUSE_FILTER_PASS if not choosing_first else Control.MOUSE_FILTER_IGNORE
		)
	if market_list != null:
...
func _render_candidates() -> void:
	...
	var title := Label.new()
	title.text = "PASO INICIAL · ELEGÍ AL PRIMER GLADIADOR DEL LUDUS"
	...
	introduction.text = (
		"Esta es la primera contratación de la campaña. Solo podés elegir a uno; "
		+ "los otros dos pasarán a casas rivales y podrán reaparecer más adelante."
	)
...
func _build_candidate_card(offer: Dictionary) -> Control:
	...
	var stats := Label.new()
	stats.text = _canonical_combat_stats_text(offer)
	...
	var choice_warning := "Esta decisión es permanente."
	choice_warning += " Los otros candidatos pasarán a casas rivales."
	choose.tooltip_text = choice_warning
...
func _canonical_combat_stats_text(offer: Dictionary) -> String:
...
func _consequence_text(gladiator_id: String) -> String:
	match gladiator_id:
		"marcus_varro":
			return (
				"Elección segura: adaptación rápida y buena popularidad. "
				+ "Odran y Neria reforzarán a tus rivales."
			)
		"odran":
			return (
				"Elección de alto impacto: gran fuerza, pero baja lealtad. "
				+ "Marcus y Neria crecerán en casas rivales."
			)
		"neria":
			return (
				"Elección técnica: movilidad, precisión y ventaja contra bestias. "
				+ "Marcus y Odran serán futuros rivales."
			)
		_:
			return "Los candidatos no elegidos serán contratados por casas rivales."
