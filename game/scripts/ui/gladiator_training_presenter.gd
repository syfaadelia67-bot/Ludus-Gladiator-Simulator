...
func _ready() -> void:
	GladiatorTrainingController.training_focus_changed.connect(
		func(person_id: String, _focus_id: String):
			if person_id == selected_person_id:
				call_deferred("_refresh")
	)
	GladiatorTrainingController.training_completed.connect(
		func(person_id: String, _result: Dictionary):
			if person_id == selected_person_id:
				call_deferred("_refresh")
	)
...
func _attach() -> void:
	for _attempt in range(90):
		...
		var dossier := (
			get_tree().current_scene.get_node_or_null("GladiatorDossier")
...
func _refresh_from_tabs(tabs: TabContainer) -> void:
	...
	var title := Label.new()
	title.text = "PLAN DE ENTRENAMIENTO MENSUAL"
...
func _refresh() -> void:
	...
	var focus_data := GladiatorTrainingController.get_focus_data(
		str(preview.get("focus_id", "balanced"))
	)
	var assignment := (
		"Asignado a Entrenamiento"
		if bool(preview.get("assigned", false))
		else "No está asignado a Entrenamiento"
	)
	var availability := (
