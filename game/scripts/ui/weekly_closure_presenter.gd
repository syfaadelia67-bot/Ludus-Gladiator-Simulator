...
func _build_overlay(scene: Node) -> void:
	...
	var title := Label.new()
	title.text = "RESUMEN ANTES DE CERRAR EL MES"
	...
	confirm_button.text = "Confirmar cierre de mes"
...
func _render() -> void:
	...
	lines.append(
		"[b]Mes %d → Mes %d[/b]"
		% [int(summary.get("month", 1)), int(summary.get("next_month", 2))]
	)
	...
	lines.append(
		"• Mantenimiento y salarios: %d" % int(economy.get("maintenance_and_wages", 0))
	)
	...
	if not blockers.is_empty():
		lines.append("\n[color=red][b]NO SE PUEDE CERRAR EL MES[/b][/color]")
	...
	confirm_button.tooltip_text = (
		"Resolvé los bloqueos indicados antes de avanzar."
		if confirm_button.disabled
		else "Procesa el mes y abre el siguiente."
	)
