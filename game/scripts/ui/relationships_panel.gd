...
func _refresh_overview() -> void:
	...
	interventions_label.text = (
		"INTERVENCIONES DEL MES %d/%d"
		% [
...
func _refresh_people() -> void:
	...
	for person in RosterManager.get_people():
		...
		var marker := (
			" · ATENCIÓN"
			if person_id in [str(pending.get("actor_id", "")), str(pending.get("target_id", ""))]
			else ""
		)
		people_list.add_item(
			(
				"%s%s\n%d vínculo(s) · %d conflicto(s)"
				% [person.display_name, marker, strong_count, conflict_count]
			)
		)
...
func _refresh_selected_summary() -> void:
	...
	if not relations.is_empty():
		...
		strongest = (
			"%s con %s" % [first.get("tone", "Neutral"), first.get("other_name", "otra persona")]
		)
...
func _refresh_detail() -> void:
	...
	for action_data in RelationshipManager.get_available_interactions(
		selected_person_id, selected_partner_id
	):
...
func _attempt_interaction(interaction_id: String, allowed: bool, blocked_reason: String) -> void:
	...
	if not allowed:
		last_feedback = (
			"NO DISPONIBLE · %s"
			% (blocked_reason if not blocked_reason.is_empty() else "No se cumplen los requisitos.")
		)
		status_label.text = last_feedback
		return
	...
	refresh_pending = false
	var result := RelationshipManager.register_interaction(
		selected_person_id, selected_partner_id, interaction_id
	)
	interaction_in_progress = false

	var success := bool(result.get("success", false))
	var description := _monthly_reason(
		str(result.get("description", "No se pudo realizar la intervención."))
	)
	if success:
		last_feedback = (
			"INTERVENCIÓN APLICADA · %s · Restan %d"
			% [
				description,
				int(
					result.get(
						"interventions_remaining", RelationshipManager.get_interventions_remaining()
					)
				)
			]
		)
...
func _resolve_incident(choice_id: String) -> void:
	...
	last_feedback = (
		"INCIDENTE RESUELTO · %s" % description if not result.is_empty() else description
	)
	_refresh()
