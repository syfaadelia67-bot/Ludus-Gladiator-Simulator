...
func _show_main_menu() -> void:
	...
	continue_button = _button(
		_t("START_VIEW_FINAL_RESULT") if campaign_over else _t("START_CONTINUE_CAMPAIGN"),
		_continue_campaign,
	)
	...
	status_label.text = (
		"%s\n%s" % [_format_save_summary(metadata), inspection_message]
		if not metadata.is_empty()
		else inspection_message
	)
...
func _format_save_summary(metadata: Dictionary) -> String:
	var month := maxi(1, int(metadata.get("month", metadata.get("week", metadata.get("day", 1)))))
	...
	var owner_line := (
		"%s %s" % [owner_title, owner_name] if not owner_name.is_empty() else owner_title
	)
	if bool(metadata.get("campaign_over", false)):
		return _t(
			"START_CAMPAIGN_FINISHED_SUMMARY",
			{
				...
				"week": mini(CampaignManager.DEMO_FINAL_MONTH, month),
				"wins": maxi(0, int(metadata.get("wins", 0))),
				"losses": maxi(0, int(metadata.get("losses", 0))),
			},
		)
	return _t(
		"START_ACTIVE_CAMPAIGN_SUMMARY",
		{
			"owner": owner_line,
			"week": month,
			"chapter": _chapter_number_for_month(month),
			"chapter_name": _chapter_name_for_month(month),
			"battle": _activity_name_for_month(month),
		},
	)
...
func _chapter_number_for_month(month: int) -> int:
	var chapter := CampaignManager.get_chapter_for_month(month)
	return maxi(1, int(chapter.get("number", 1)))


func _chapter_name_for_month(month: int) -> String:
	var chapter := CampaignManager.get_chapter_for_month(month)
	var number := maxi(1, int(chapter.get("number", 1)))
	return _t("CHAPTER_%d_NAME" % number)


func _activity_name_for_month(month: int) -> String:
...
func _capture_owner_draft() -> void:
	...
	if (
		title_selector != null
		and is_instance_valid(title_selector)
		and title_selector.selected >= 0
	):
		owner_draft["title_id"] = str(title_selector.get_item_metadata(title_selector.selected))
	if (
		origin_selector != null
		and is_instance_valid(origin_selector)
		and origin_selector.selected >= 0
	):
