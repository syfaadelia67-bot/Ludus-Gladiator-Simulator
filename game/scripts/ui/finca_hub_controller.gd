...
func open_gladiator_dossier(
	person_id: String, context: Dictionary = {}, initial_tab: String = "information"
) -> bool:
	...
	for child in host.get_children():
		if child is Control:
			...
			control.mouse_filter = (
				Control.MOUSE_FILTER_PASS if active else Control.MOUSE_FILTER_IGNORE
			)
...
func _open_arena_dossier(arena_screen: Control, initial_tab: String) -> void:
	...
	if person_id.is_empty():
		...
		return
	open_gladiator_dossier(person_id, {"system_id": "arena", "selected_id": person_id}, initial_tab)
