...
signal polygons_updated(polygons : Array[PackedVector2Array], poly_strokes : Array[PackedVector2Array], myself : Node2D)
...
signal transform_changed(ref_to_self : Node2D)
...
func _update_curve():
	...
	var valid_clip_paths : Array = (clip_paths
...
func _update_assigned_nodes_with_clips(polygon_points : PackedVector2Array, valid_clip_paths : Array) -> void:

	var merges := valid_clip_paths.filter(func(cp : Node2D): return cp.use_union_in_stead_of_clipping)
	var clips := valid_clip_paths.filter(func(cp : Node2D): return cp.use_interect_when_clipping)
	var cutouts := valid_clip_paths.filter(func(cp : Node2D): return not cp.use_interect_when_clipping and not cp.use_union_in_stead_of_clipping)
...
func _clip_path_to_local(clip_path : Node2D) -> PackedVector2Array:
	var pts: PackedVector2Array = clip_path.global_transform * clip_path.tessellate()
